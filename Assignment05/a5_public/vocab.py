...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        # sents : [ [ 'i', 'am', 'a', 'boy'], ['you', 'are', 'a', 'girl'], ..., ]
        # word_ids : [ [ [1], [2, 4], [2], [5, 6, 7] ], [ ,, ] ... ]
        return [[[self.char2id[char] for char in '{' + word + '}'] for word in sen] for sen in sents]
        # END YOUR CODE
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents_padded = pad_sents_char(words_ids)
        sents_var = torch.Tensor(data=sents_padded, device=device)
