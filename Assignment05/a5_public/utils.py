...
def pad_sents_char(sents, char_pad_token):
    ...
    max_sen_len = max([len(sen) for sen in sents])
    for sen in sents:
        for idx, word in enumerate(sen):
            sen[idx] = word[:max_word_length] + ([char_pad_token] * ( max(max_word_length - len(word), 0) ))
        sen += [[char_pad_token] * max_word_length] * (max_sen_len - len(sen))
    sents_padded = sents
    ### END YOUR CODE
